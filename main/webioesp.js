...
function w() {
	if (_webioesp == undefined) {
		_webioesp = new WebIOEsp();
	}
...
function WebIOEsp() {
	...
	this.TYPE = {
			...
			RST: {value: 2, style: "RST", label: "RESET"},
			V33: {value: 3, style: "V33", label: "3.3V"},
			V50: {value: 4, style: "V50", label: "5.0V"},
			GPIO: {value: 5, style: "GPIO", label: "GPIO"}
	};
	...
	*/
	...
	// get context
	var reg = new RegExp("http://" + window.location.host + "(.*)webioesp.js");
...
*/
	...
	this.addALT(this.ALT.UART, 43, "TX");
	this.addALT(this.ALT.UART, 44, "RX");

	this.addALT(this.ALT.ONEWIRE, 4, "");
}

WebIOEsp.prototype.init = function() {
	$.getJSON(w().context + "pinout", function(data) {
...
/*
WebIOEsp.prototype.initMobile = function() {
	webioesp().init();
}
*/

WebIOEsp.prototype.ready = function (cb) {
	w().readyCallback = cb;
}

WebIOEsp.prototype.map = function (pin, type, value) {
	w().PINS[pin] = Object();
...
WebIOEsp.prototype.addALT = function (alt, gpio, name) {
	var o = Object();
	o.gpio = gpio;
	o.name = name;
	alt.gpios.push(o);
...
WebIOEsp.prototype.updateValue = function (gpio, value) {
	w().GPIO[gpio].value = value;
	var style = (value == 1) ? "HIGH" : "LOW";
	$("#gpio"+gpio).attr("class", style);
}

WebIOEsp.prototype.updateFunction = function (gpio, func) {
	w().GPIO[gpio].func = func;
	$("#function"+gpio).val(func);
	$("#function"+gpio).text(func);
}

WebIOEsp.prototype.updateSlider = function (gpio, slider, value) {
	$("#"+slider+gpio).val(value);
}

WebIOEsp.prototype.updateALT = function (alt, enable) {
...
WebIOEsp.prototype.refreshGPIO = function (repeat) {
	$.getJSON(w().context + "gpio_getall", function(data) {
...
WebIOEsp.prototype.getDeviceInfo = function () {
	$.getJSON(w().context + "deviceinfo", function(data) {
		
		var pincount = parseInt(data[3]);
...
WebIOEsp.prototype.checkVersion = function () {
	var version;
...
//
//		$.get("http://webioesp.trouch.com/version.php", function(data) {
...
WebIOEsp.prototype.digitalRead = function (gpio, callback) {
...
WebIOEsp.prototype.digitalWrite = function (gpio, value, callback) {
	if (w().GPIO[gpio].func.toUpperCase()=="OUT") {
		$.get(w().context + 'gpio_ctrl?port=' + gpio + "&value=" + value, function(data) {
			w().updateValue(gpio, data);
			if (callback != undefined) {
				callback(gpio, data);
			}
...
WebIOEsp.prototype.getFunction = function (gpio, callback) {
	if (callback != undefined) {
		$.get(w().context + 'GPIO1?port=' + gpio + "&func=", function(data) {
...
WebIOEsp.prototype.setFunction = function (gpio, func, callback) {
	$.get(w().context + 'gpio_ctrl?port=' + gpio + "&func=" + func, function(data) {
		w().updateFunction(gpio, data);
		if (callback != undefined) {
			callback(gpio, data);
		}
...
WebIOEsp.prototype.toggleValue = function (gpio) {
	var value = (w().GPIO[gpio].value == 1) ? 0 : 1;
	w().digitalWrite(gpio, value);
}

WebIOEsp.prototype.toggleFunction = function (gpio) {
	var value = (w().GPIO[gpio].func == "IN") ? "OUT" : "IN";
	w().setFunction(gpio, value)
}

WebIOEsp.prototype.outputSequence = function (gpio, period, sequence, callback) {
...
WebIOEsp.prototype.callMacro = function (macro, args, callback) {
	if (args == undefined) {
		args = "";
	}
...
WebIOEsp.prototype.enablePWM = function(gpio, callback) {
...
WebIOEsp.prototype.disablePWM = function(gpio, callback) {
...
WebIOEsp.prototype.pulse = function(gpio, callback) {
...
WebIOEsp.prototype.pulseRatio = function(gpio, ratio, callback) {
...
WebIOEsp.prototype.pulseAngle = function(gpio, angle, callback) {
...
WebIOEsp.prototype.setLabel = function (id, label) {
	$("#" + id).val(label);
	$("#" + id).text(label);
}

WebIOEsp.prototype.setClass = function (id, cssClass) {
	$("#" + id).attr("class", cssClass);
}

WebIOEsp.prototype.createButton = function (id, label, callback, callbackUp) {
...
WebIOEsp.prototype.createGPIOButton = function (gpio, label) {
...
WebIOEsp.prototype.createFunctionButton = function (gpio) {
...
WebIOEsp.prototype.createPulseButton = function (id, label, gpio) {
    var button = webioesp().createButton(id, label);
    button.bind(BUTTON_DOWN, function(event) {
        webioesp().pulse(gpio);
    });
    return button;
...
WebIOEsp.prototype.createMacroButton = function (id, label, macro, args) {
    var button = webioesp().createButton(id, label);
    button.bind(BUTTON_DOWN, function(event) {
        webioesp().callMacro(macro, args);
    });
    return button;
}

WebIOEsp.prototype.createSequenceButton = function (id, label, gpio, period, sequence) {
    var button = webioesp().createButton(id, label);
    button.bind(BUTTON_DOWN, function(event) {
        webioesp().outputSequence(gpio, period, sequence);
    });
    return button;
}

WebIOEsp.prototype.createRatioSlider = function(gpio) {
...
WebIOEsp.prototype.createAngleSlider = function(gpio) {
...
WebIOEsp.prototype.ESPHeader = function () {
	if (w()._header == undefined) {
		w()._header = new ESPHeader();
	}
	return w()._header;
}
...
ESPHeader.prototype.getPinCell = function (pin) {
...
ESPHeader.prototype.getDescriptionCell = function (pin, align) {
	var cell = $('<td>');
...
ESPHeader.prototype.getFunctionCell = function (pin) {
	var cell = $('<td align="center">');
	if (w().PINS[pin].type.value == w().TYPE.GPIO.value) {
...
ESPHeader.prototype.createTable = function (containerId) {
	var table = $("<table>");
	table.attr("id", "ESPHeader")
...
WebIOEsp.prototype.Expert = function () {
...
WebIOEsp.prototype.Serial = function(device) {
...
WebIOEsp.prototype.newDevice = function(type, name) {
	if (type == "ADC") {
		return new ADC(name);
	}
...
GPIOPort.prototype.refreshUI = function() {
	...
	if ((element != undefined) && (element.table == undefined) && this.isReady()) {
		...
		for (var i = this.channelCount-1; i>=0; i--) {
			var cell = $("<td>");
			var button = webioesp().createButton(this.name + "_" + i + "_value", i, function() {
		...
		for (var i = this.channelCount-1; i>=0; i--) {
			var cell = $("<td>");
			var button = webioesp().createButton(port.name + "_" + i + "_func", "IN", function() {
				var func = $(this).text();
				console.log(func);
				if (func == "IN") {
					func = "OUT";
...
PiFaceDigital.prototype.refreshUI = function() {
	...
	if ((element != undefined) && (element.table == undefined)) {
		...
		for (var i = 7; i>=0; i--) {
			var cell = $("<td>");
			var button = webioesp().createButton(this.name + "_output_" + i, i, function() {
		...
		for (var i = 7; i>=0; i--) {
			var cell = $("<td>");
			var button = webioesp().createButton(this.name + "_input_" + i, i, function() {
			});
			button.attr("channel", i);
			button.attr("class", "LOW");
			cell.append(button);
